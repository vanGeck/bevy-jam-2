...
pub struct CraftGridConfig {
    pub coords: Coords,
    pub tile_size: i32,
}

impl CraftGridConfig {
    ...
    #[must_use]
    pub fn load_from_file() -> CraftGridConfig {
...
fn load_from_path(path: &Path) -> CraftGridConfig {
    fs::read_to_string(path)
        .and_then(|data| ron::de::from_str::<CraftGridConfig>(&data).map_err(|error| Error::new(ErrorKind::Other, error)))
        .unwrap_or_else(|error| {
            error!(
                    "Failed to load the grid config file from {:?}! Falling back to GridConfig::default(). Error: {:?}",
                    path, error
                );
            CraftGridConfig::default()
        })
}
