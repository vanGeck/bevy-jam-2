...
use bevy::prelude::{Commands, ResMut, Component};
...
pub fn tick_temporary_modifiers(mut q: Query<(Entity, &mut TemporaryModifier)>, 
                                mut hero: ResMut<Hero>, 
                                mut enemy: ResMut<Enemy>,
                                mut ticker: ResMut<TimedEffectTicker>, 
                                time: Res<Time>, mut cmd: Commands) {
...
pub fn test_apply_modifier(input: Res<Input<KeyCode>>, mut cmd: Commands) {
    if input.just_pressed(KeyCode::M){
        apply_timed_modifier(TemporaryModifier{
            time: 10.0,
            damage_mod: 4,
            on_hero: true,
            ..default()
